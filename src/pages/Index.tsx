    ...
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 text-center max-w-4xl mx-auto">
        <div className="bg-[#4CAF50]/10 p-4 rounded-full mb-6">
          <Heart className="w-12 h-12 text-[#4CAF50] mb-3" />
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4 leading-tight">Never miss a dose again</h1>
        <p className="text-lg text-gray-600 mb-8 max-w-2xl">A smart medicine companion that ensures you never miss a dose — with family support and zero effort.</p>
        <div className="flex flex-col sm:flex-row gap-4 mb-12">
          <Link to="/signup">
            <Button size="lg" className="bg-[#4CAF50] hover:bg-[#43A047] text-white rounded-xl px-8 h-12 text-base font-medium shadow-lg shadow-[#4CAF50]/20">Create Free Account</Button>
          </Link>
          <Link to="/login">
            <Button size="lg" variant="outline" className="rounded-xl px-8 h-12 text-base font-medium border-gray-300 hover:bg-gray-50">Sign In</Button>
          </Link>
        </div>
      ...
      <footer className="p-6 text-center text-sm text-gray-400">
        &copy; {new Date().getFullYear()} MediMind. Built with care for better health.
      </footer>
    </div>
  );
};
